...
docker ps --filter name=${CURRENT_HL_TOPOLOGY}-orderers --filter status=running -aq | xargs -r docker stop | xargs -r docker rm
docker ps --filter name=${CURRENT_HL_TOPOLOGY}-orderers --filter status=exited -aq | xargs -r docker rm
set -e
