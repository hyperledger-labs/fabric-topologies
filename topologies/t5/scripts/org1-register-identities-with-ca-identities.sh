...
fabric-ca-client enroll -d -u https://org1-ca-identities-admin:org1-ca-identities-adminpw@0.0.0.0:9000
...
fabric-ca-client register -d --id.name admin-org1 --id.secret org1adminpw --id.type admin --id.attrs "hf.Registrar.Roles=client,hf.Registrar.Attributes=*,hf.Revoker=true,hf.GenCRL=true,admin=true:ecert,abac.init=true:ecert" -u https://0.0.0.0:9000